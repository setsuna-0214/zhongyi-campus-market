...
class HomeControllerTest {
        ...
        void testGetHot_DefaultLimit() throws Exception {
                HomeDto.HomeProduct product = new HomeDto.HomeProduct(
                                1, "热门商品", "http://img.jpg", null, 99,
                                "", null, 1, "卖家", "北京", null, "在售", 100);
                List<HomeDto.HomeProduct> products = Arrays.asList(product);

                when(homeService.getHotProducts(1, 10)).thenReturn(products);
                ...
                verify(homeService, times(1)).getHotProducts(1, 10);
        }
        ...
        void testGetHot_CustomLimit() throws Exception {
                when(homeService.getHotProducts(1, 5)).thenReturn(Collections.emptyList());

                mockMvc.perform(get("/home/hot").param("pageSize", "5"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$").isArray());

                verify(homeService, times(1)).getHotProducts(1, 5);
        }
        ...
        void testGetHot_EmptyResult() throws Exception {
                when(homeService.getHotProducts(1, 10)).thenReturn(Collections.emptyList());
        ...
        void testGetLatest_DefaultLimit() throws Exception {
                HomeDto.HomeProduct product = new HomeDto.HomeProduct(
                                100, "最新商品", "http://img.jpg", null, 199,
                                null, "", 2, "卖家B", "上海", null, "在售", 10);
                List<HomeDto.HomeProduct> products = Arrays.asList(product);

                when(homeService.getLatestProducts(1, 10)).thenReturn(products);
                ...
                verify(homeService, times(1)).getLatestProducts(1, 10);
        }
        ...
        void testGetLatest_CustomLimit() throws Exception {
                when(homeService.getLatestProducts(1, 20)).thenReturn(Collections.emptyList());

                mockMvc.perform(get("/home/latest").param("pageSize", "20"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$").isArray());

                verify(homeService, times(1)).getLatestProducts(1, 20);
        }
